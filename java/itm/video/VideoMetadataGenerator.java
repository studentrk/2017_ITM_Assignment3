...
public class VideoMetadataGenerator {
	...
	protected VideoMedia processVideo(File input, File output, boolean overwrite) throws Exception {
		...
		// create IContainer, which contains many streams
        IContainer iContainer = IContainer.make();

        // open and check if the Container is opened
        if((iContainer.open(input.toString(), IContainer.Type.READ, null)) < 0)
            throw new IllegalArgumentException("Cannot read the file.");

        int numberOfStreams = iContainer.getNumStreams();

        // current stream object
        IStream iStream = null;

        // configured decoder to decode all streams
        IStreamCoder coder = null;

		// The length is in seconds
		media.setVideoLength(iContainer.getDuration()/1000000);

        // loop through all streams
        for(int i = 0; i < numberOfStreams; i++)
        {
            // get the current stream
            iStream = iContainer.getStream(i);

            // decode the current stream
            coder = iStream.getStreamCoder();
            ...
            else
            {
                ...
                media.setAudioBitRate(coder.getBitRate() / 1000);
            }
        }
